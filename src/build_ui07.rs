...
pub fn build_ui(application: &gtk::Application) {
    ...
    let wtitle = format!("Photo Organize, Merge and Convert Rust GTK4 version: {}.{}.{}",gtk::get_major_version(), gtk::get_minor_version(), gtk::get_micro_version());
...
// --------- ORGANIZE TWO DIRECTORIES ---------------
    ...
    gtk::WidgetExt::set_widget_name(&odirectory1_button, "odir1");
    ...
    gtk::WidgetExt::set_widget_name(&odirectory2_button, "odir2");
    ...
    gtk::WidgetExt::set_widget_name(&odate_compare_button, "ocomp");
    let opreview_button = Button::with_label("Preview Images");
    gtk::WidgetExt::set_widget_name(&opreview_button, "oprev");
    ...
    gtk::WidgetExt::set_widget_name(&odirectory_o_button, "oodir");
    ...
    v1box.attach(&oprevimage1_label, 0, 3 , 2, 1);
    ...
    v1box.attach(&oprevimage2_label, 8, 3 , 2, 1);
    v1box.attach(&ocurrimage1_label, 0, 4 , 2, 1);
    v1box.attach(&oscroll_window1, 2, 3 , 3, 3);
    v1box.attach(&oscroll_window2, 5, 3 , 3, 3);
    v1box.attach(&ocurrimage2_label, 8, 4 , 2, 1);
    v1box.attach(&oafterimage1_label, 0, 5 , 2, 1);
    v1box.attach(&oafterimage2_label, 8, 5 , 2, 1);
    ...
    gtk::WidgetExt::set_widget_name(&tab1_label, "tab1");
...
// --- TAB 2 MERGE INTO DIRECTORY ------------------
    ...
    v2box.attach(&mscrollfrom_window, 0, 4 , 2, 6);
    v2box.attach(&mscrollto_window, 2, 4 , 2, 6);
    v2box.attach(&mprevimageto_label, 4, 4 , 1, 2);
    v2box.attach(&mcurrimageto_label, 4, 6 , 1, 2);
    v2box.attach(&mafterimageto_label, 4, 8 , 1, 2);

    let tab2_label = Label::new(Some("Merge into directory"));
    tab2_label.set_width_chars(30);
    gtk::WidgetExt::set_widget_name(&tab2_label, "tab2");
...
// --- TAB 3 CONVERT DIRECTORY ------------------
    ...
    v3box.attach(&cdirectory1_combobox, 1, 1 , 9, 1);
    v3box.attach(&cscroll_window1, 0, 2 , 10, 2);
    ...
    gtk::WidgetExt::set_widget_name(&tab3_label, "tab3");
            ...
            if let Some(cur_dir1) = odirectory1_combobox.get_active_text() {
                ...
                if treemodel1 == None {
                    ...
                    if let Some((modelt1, itert1)) = selectiont1.get_selected() {
                        let tofilenameval1 = modelt1.get(&itert1, 0).get::<String>();
            ...
            if let Some(cur_dir2) = odirectory2_combobox.get_active_text() {
                ...
                if treemodel2 == None {
                    ...
                    if let Some((modelt2, itert2)) = selectiont2.get_selected() {
                        let tofilenameval2 = modelt2.get(&itert2, 0).get::<String>();
...
// check if merge files and if so process
        if bolok {
            ...
            if view3model == None {
                ...
                while valid {
                      let dirval = view3modeluw.get(&validval,0).get::<String>();
                      ...
                      let filefromval = view3modeluw.get(&validval,1).get::<String>();
                      ...
                      let filepreval = view3modeluw.get(&validval,2).get::<String>();
                      ...
                      let filetoval = view3modeluw.get(&validval,3).get::<String>();
            ...
            if treemodel1 == None {
                ...
                if let Some((modelt1, itert1)) = selectiont1.get_selected() {
                    ...
                    if !odir1chkbox_check.get_active() {
                        let takedate1 = modelt1.get(&itert1, 1).get::<String>();
                        let takedatestr1 = format!("{:?}", takedate1);
                        if takedatestr1.len() < 31 {
                            let imgdate1 = modelt1.get(&itert1, 2).get::<String>();
                            let imgdatestr1 = format!("{:?}", imgdate1);
                            if imgdatestr1.len() < 31 {
                                let moddate1 = modelt1.get(&itert1, 3).get::<String>();
                ...
                if treemodel2 == None {
                    ...
                    if let Some((modelt2, itert2)) = selectiont2.get_selected() {
                        ...
                        if !odir2chkbox_check.get_active() {
                            let takedate2 = modelt2.get(&itert2, 1).get::<String>();
                            let takedatestr2 = format!("{:?}", takedate2);
                            if takedatestr2.len() < 31 {
                                let imgdate2 = modelt2.get(&itert2, 2).get::<String>();
                                let imgdatestr2 = format!("{:?}", imgdate2);
                                if imgdatestr2.len() < 31 {
                                    let moddate2 = modelt2.get(&itert2, 3).get::<String>();
    ...
    mdirectoryfrom_button.connect_clicked(glib::clone!(@weak window, @weak mdirectoryfrom_combobox, @weak mdirectoryto_combobox, @weak messageval_label, @weak miconfrom_view, @weak mimsize_entry => move|_| {
        ...
        if badsize_int == 0 {
            ...
            dialog.connect_response(move |d: &FileChooserDialog, response: gtk::ResponseType| {
                ...
                if messageval_label.get_text() == "getting directory From" {
...
//----------------- Merge To directory button start -----------------------------------
    mdirectoryto_button.connect_clicked(glib::clone!(@weak window, @weak mdirectoryto_combobox, @weak messageval_label, @weak mtreeto_view => move|_| {
        ...
        dialog.connect_response(move |d: &FileChooserDialog, response: gtk::ResponseType| {
            ...
            if messageval_label.get_text() == "getting directory To" {
                messageval_label.set_markup("<span color=\"#FF000000\">********* From Directory: ERROR  OPEN  button not selected **********</span>");
            }
            d.close();
        });
...
//----------------- Merge merge button start -----------------------------------
    mmerge_button.connect_clicked(glib::clone!(@weak mdirectoryto_combobox, @weak miconfrom_view, @weak mdirectoryfrom_combobox, @weak mtreeto_view, @weak mbeforebox_check, @weak messageval_label  => move|_| {
...
// check a from file has been selected and that there is only one selection
        if bolok {
            ...
            if icon_selectpath.len() == 1 {
                ...
                let filenameval = iconmodeluw.get(&iconiter,1).get::<String>();
                let filenamestr = format!("{:?}", filenameval).to_owned();
...
// check if to file was selected - no check for number just get first one
        if bolok {
            ...
            if let Some((modelt, itert)) = selectiont.get_selected() {
                let tofilenameval = modelt.get(&itert, 0).get::<String>();
...
//----------------- convert out directory button start -----------------------------------
    cdirectory_o_button.connect_clicked(glib::clone!(@weak window, @weak cdirectory_o_combobox, @weak messageval_label => move|_| {
        ...
        dialog.connect_response(move |d: &FileChooserDialog, response: gtk::ResponseType| {
            ...
            if messageval_label.get_text() == "getting out directory" {
