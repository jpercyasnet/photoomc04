...
pub fn gen_merge (datenumother: i32, mbeforebox_check: bool, datenumto: i32, dateto: DateTime<Utc>, dateother:  DateTime<Utc>) -> (u32, String, bool, i32,  DateTime<Utc>) {
...
// change to reduce date by one sec and make 500 dateto - Duration::seconds(1)
                datetox = dateto - Duration::seconds(1);
                datenumtox = 500;
        ...
        if dateto == dateother {
            if mbeforebox_check {
                if (datenumto - datenumother) < 2 {
                    errstring = "<span color=\"#FF000000\">********* Merge: before checked & selected file number & previous file number less than 2 apart **********</span>".to_string();
                    errcode = 3;
                } else {
                ...
                if (datenumother - datenumto) < 2 {
                    errstring = "<span color=\"#FF000000\">********* Merge: selected file number & next file number less than 2 apart **********</span>".to_string();
                    errcode = 4;
                } else {
...
// dates not the same
            ...
            if !bolusenum {
                ...
                if durationn.num_seconds() < 2 {
                    if mbeforebox_check {
                        if datenumother > 998 {
                            errstring = "<span color=\"#FF000000\">********* Merge: before checked & selected file number & previous file number too close **********</span>".to_string();
                            errcode = 5;
                        } else {
                            datetox = dateother;
                        ...
                        if datenumother < 1 {
                            errstring = "<span color=\"#FF000000\">********* Merge: selected file number & after file number too close **********</span>".to_string();
                            errcode = 6;
                        } else {
                            datetox = dateother;
                            if datenumother > 1 {
                    ...
                    if mbeforebox_check {
                        datetox = dateother + Duration::seconds(sec/2);
                    } else {
                        datetox = dateother - Duration::seconds(sec/2);
                    }
                }
