...
fn get_tomodel(cur_dir: PathBuf) -> (u32, String, ListStore) { 
...
//                       if valid date get the orientation value
                                 ...
                                 if let Err(e) = dump_file(&file_path) {
                                     ...
                                     if let Some(field) = reader.get_field(Tag::Orientation, false) {
...
fn get_prevafter(cur_dir: String, tofilename: String) -> (u32, String, String, String) { 
    let mut errcode: u32 = 0;
    let mut errstring: String = " ".to_string();
    ...
    namepo = namep.to_string();
    nameao = namea.to_string();
...
// input is the directory and output is error number, error string and model
fn get_dirmodel (current_dir: PathBuf) -> (u32, String, ListStore) {
    let mut errcode: u32 = 0;
    let mut errstring: String = " ".to_string();
             ...
             if let Ok(file_name) = entry.file_name().into_string() {
                 if metadata.is_file() {
                     ...
                     if let Err(e) = dump_file(&file_path) {
                         ...
                         if let Some(field1) = reader.get_field(Tag::DateTimeOriginal, false) {
                             file_date = format!("{}",field1.value.display_as(field1.tag));
                             date_from = format!("date taken");
                         } else {
                             if let Some(field2) = reader.get_field(Tag::DateTime, false) {
...
fn get_strvector(current_dir: PathBuf, dirnum: i32, filesize: i32, dateinfile: bool, dateyrx: i64, datedayx: i64, datemox: i64, datehrx: i64, dateminx: i64, datesecx: i64) -> (u32, String, Vec<std::string::String>) {
    let mut errcode: u32 = 0;
    let mut errstring: String = " ".to_string();
    let mut listitems: Vec<String> = Vec::new();
    let mut orient = format!("-");
    let mut file_prefixdate = format!(" ");
...
//    if exif data first set date to DateTimeOriginal, if not DateTime and add mod date. if any errors, still have modified date
                             ...
                             if let Some(field1) = reader.get_field(Tag::DateTimeOriginal, false) {
                                 ...
                                 if let Some(field1) = reader.get_field(Tag::DateTime, false) {
                                     exifdate = format!("{}",field1.value.display_as(field1.tag));
                                 }
                             }
                             if exifdate != format!(" ") {
                                 ...
                                 if baddatefdx == 0 {
                                     ...
                                     datetox = Utc.ymd(dateyr, datemo, dateday).and_hms_milli(datehr as u32, datemin as u32, datesec as u32, 0);
...
fn dump_file(path: &Path) -> Result<(), exif::Error> {
    ...
    let _reader = exif::Reader::new(&mut BufReader::new(&file))?;
...
fn main() {
    ...
    window.set_title("Photo Organize, Merge and Convert Rust GTK");
...
// --------- ORGANIZE TWO DIRECTORIES ---------------
    ...
    let odirectory1_button = Button::new_with_label("In Directory 1");
...
//    let oscroll_window1 = ScrolledWindow::new(gtk::NONE_ADJUSTMENT , gtk::NONE_ADJUSTMENT);
    let oscroll_window1 = ScrolledWindow::new(None , None);
...
//    let oscroll_window2 = ScrolledWindow::new(gtk::NONE_ADJUSTMENT , gtk::NONE_ADJUSTMENT);
    let oscroll_window2 = ScrolledWindow::new(None , None);
...
//    let oscroll_window3 = ScrolledWindow::new(gtk::NONE_ADJUSTMENT , gtk::NONE_ADJUSTMENT);
    let oscroll_window3 = ScrolledWindow::new(None , None);
    oscroll_window3.add(&otree_view3);
    oscroll_window3.set_hexpand(true);
    oscroll_window3.set_vexpand(true);

    let odirectory2_button = Button::new_with_label("In Directory 2");
    let odirectory2_combobox = ComboBoxText::new();
    odirectory2_combobox.set_hexpand(true);

    let odir1chkbox_check = CheckButton::new_with_label("Date in Filename 1");
    let odir2chkbox_check = CheckButton::new_with_label("Date in Filename 2");
    ...
    let odate_compare_button = Button::new_with_label("Compare Dates");
    let opreview_button = Button::new_with_label("Preview Images");
    ...
    let odirectory_o_button = Button::new_with_label("outDirectory");
    let odirectory_o_combobox = ComboBoxText::new();
    odirectory_o_combobox.set_hexpand(true);

    let omerge_button = Button::new_with_label("Merge");
    let ocopy_button = Button::new_with_label("Copy");
...
// --- TAB 2 MERGE INTO DIRECTORY ------------------
    ...
    let mdirectoryfrom_button = Button::new_with_label("fromDir");
    let mdirectoryfrom_combobox = ComboBoxText::new();
    mdirectoryfrom_combobox.set_hexpand(true);

    let mdirectoryto_button = Button::new_with_label("ToDir");
    let mdirectoryto_combobox = ComboBoxText::new();
    mdirectoryto_combobox.set_hexpand(true);

    let mbeforebox_check = CheckButton::new_with_label("Before (otherwise After)");

    let mmerge_button = Button::new_with_label("Merge");
...
//    let mscrollfrom_window = ScrolledWindow::new(gtk::NONE_ADJUSTMENT , gtk::NONE_ADJUSTMENT);
    let mscrollfrom_window = ScrolledWindow::new(None , None);
...
//    let mscrollto_window = ScrolledWindow::new(gtk::NONE_ADJUSTMENT , gtk::NONE_ADJUSTMENT);
    let mscrollto_window = ScrolledWindow::new(None , None);
...
//    let afterimageto_label = Label::new("after image");
    ...
    let mpreview_button = Button::new_with_label("Preview Images");
...
// --- TAB 3 CONVERT DIRECTORY ------------------
    ...
    let cdirectory1_button = Button::new_with_label("inDir");
...
//    let cscroll_window1 = ScrolledWindow::new(gtk::NONE_ADJUSTMENT , gtk::NONE_ADJUSTMENT);
...
//    let cscroll_window3 = ScrolledWindow::new(gtk::NONE_ADJUSTMENT , gtk::NONE_ADJUSTMENT);
    let cscroll_window3 = ScrolledWindow::new(None , None);
    ...
    let cdirectory_o_button = Button::new_with_label("outDirectory");
    let cdirectory_o_combobox = ComboBoxText::new();
    cdirectory_o_combobox.set_hexpand(true);

    let cmerge_button = Button::new_with_label("Merge");
    let ccopy_button = Button::new_with_label("Copy");
...
//-------------- connects
// need to be able to do hortizonal scrolling for list to reduce window width
//
//  *A done  let odirectory1_button = Button::new_with_label("inDir1"); o1
//  *A done let odirectory2_button = Button::new_with_label("inDir2"); o2
//  -  done let odate_compare_button = Button::new_with_label("Compare Dates"); or
//  *D done let opreview_button = Button::new_with_label("Preview Images"); op
//  * delete done let oclearpreview_button = Button::new_with_label("Clear Preview images");
//  * - done let odirectory_o_button = Button::new_with_label("outDirectory"); od
//  *B done  let omerge_button = Button::new_with_label("Merge"); om
//  *-  done let ocopy_button = Button::new_with_label("Copy");  oc cannot pass progress bar
//
//
// reduce widow width with window horitzonal scrolling or reduce orient width
//
// *done    let mdirectoryfrom_button = Button::new_with_label("fromDir");
//  *done  let mdirectoryto_button = Button::new_with_label("ToDir");
// *done    let mmerge_button = Button::new_with_label("Merge");
//  *D done let mpreview_button = Button::new_with_label("Preview Images");
//
//
//  *A done let cdirectory1_button = Button::new_with_label("inDir");
//  *- done let cdirectory_o_button = Button::new_with_label("outDirectory");
//  *B  done let cmerge_button = Button::new_with_label("Merge");
//  *-  done let ccopy_button = Button::new_with_label("Copy"); cannot pass progress bar

//----------------- org directory 1 button start -----------------------------------
    let window_weako1 = window.downgrade();    
    let odirectory1_combobox_weako1 = odirectory1_combobox.downgrade();
    let messageval_label_weako1 = messageval_label.downgrade();
    let otree_view1_weako1 = otree_view1.downgrade();

    odirectory1_button.connect_clicked(clone!(window_weako1, odirectory1_combobox_weako1, messageval_label_weako1, otree_view1_weako1 => move|_| {
        let window = upgrade_weak!(window_weako1);
        let odirectory1_combobox = upgrade_weak!(odirectory1_combobox_weako1);
        let messageval_label = upgrade_weak!(messageval_label_weako1);
        let otree_view1 = upgrade_weak!(otree_view1_weako1);

        let dialog = FileChooserDialog::new(Some("Choose a Directory 1"), Some(&window.clone()), FileChooserAction::SelectFolder);
        ...
        if result == RESPONSE_ACCEPT {
            if let Some(uri) = dialog.get_uri() {
                ...
                odirectory1_combobox.prepend_text(&foldername);
                odirectory1_combobox.set_active(0);
                ...
                messageval_label.set_markup("<span color=\"#FF000000\">********* Directory 1: ERROR GETTING URI **********</span>");
            }
        } else {
            messageval_label.set_markup("<span color=\"#FF000000\">********* Directory 1: ERROR accept button not selected **********</span>");
        }
        dialog.destroy();
    }));
...
//----------------- org directory 2 button start -----------------------------------
    ...
    let window_weako2 = window.downgrade();    
    let odirectory2_combobox_weako2 = odirectory2_combobox.downgrade();
    let messageval_label_weako2 = messageval_label.downgrade();
    let otree_view2_weako2 = otree_view2.downgrade();

    odirectory2_button.connect_clicked(clone!(window_weako2, odirectory2_combobox_weako2, messageval_label_weako2, otree_view2_weako2 => move|_| {
        let window = upgrade_weak!(window_weako2);
        let odirectory2_combobox = upgrade_weak!(odirectory2_combobox_weako2);
        let messageval_label = upgrade_weak!(messageval_label_weako2);
        let otree_view2 = upgrade_weak!(otree_view2_weako2);

        let dialog = FileChooserDialog::new(Some("Choose a Directory 2"), Some(&window.clone()), FileChooserAction::SelectFolder);
        dialog.add_button("Cancel", RESPONSE_CANCEL);
        dialog.add_button("Accept", RESPONSE_ACCEPT);
        let result = dialog.run();
        if result == RESPONSE_ACCEPT {
            if let Some(uri) = dialog.get_uri() {
                let foldername = &uri[7..];
                odirectory2_combobox.prepend_text(&foldername);
                odirectory2_combobox.set_active(0);
                let current_dir = PathBuf::from(&foldername);
                let (errcd, errstr, newmodel) = get_dirmodel(current_dir);
                if errcd == 0 {
                ...
                messageval_label.set_markup("<span color=\"#FF000000\">********* Directory 2: ERROR GETTING URI **********</span>");
            }
        } else {
            messageval_label.set_markup("<span color=\"#FF000000\">********* Directory 2: ERROR accept button not selected **********</span>");
        }
        dialog.destroy();
    }));
...
//----------------- org out directory button start -----------------------------------
    ...
    let window_weakod = window.downgrade();    
    let odirectory_o_combobox_weakod = odirectory_o_combobox.downgrade();
    let messageval_label_weakod = messageval_label.downgrade();

    odirectory_o_button.connect_clicked(clone!(window_weakod, odirectory_o_combobox_weakod, messageval_label_weakod => move|_| {
        let window = upgrade_weak!(window_weakod);
        let odirectory_o_combobox = upgrade_weak!(odirectory_o_combobox_weakod);
        let messageval_label = upgrade_weak!(messageval_label_weakod);
        ...
        if result == RESPONSE_ACCEPT {
            if let Some(uri) = dialog.get_uri() {
                let foldername = &uri[7..];
                odirectory_o_combobox.prepend_text(&foldername);
                odirectory_o_combobox.set_active(0);
                messageval_label.set_text("org out directory selected");
            } else { 
                messageval_label.set_markup("<span color=\"#FF000000\">********* org Out Directory: ERROR GETTING URI **********</span>");
            }
        } else {
        messageval_label.set_markup("<span color=\"#FF000000\">********* org Out Directory: ERROR accept button not selected **********</span>");
        }
        dialog.destroy();
    }));
...
//----------------- org preview button start -----------------------------------
    ...
    opreview_button.connect_clicked(clone!(odirectory1_combobox_weakop, otree_view1_weakop, oprevimage1_label_weakop, ocurrimage1_label_weakop, oafterimage1_label_weakop, odirectory2_combobox_weakop, otree_view2_weakop, oprevimage2_label_weakop, ocurrimage2_label_weakop, oafterimage2_label_weakop, messageval_label_weakop, oimsize_entry_weakop  => move|_| {
        ...
        if let Some(inputic_text) = oimsize_entry.get_text() { 
            ...
            messageval_label.set_markup("<span color=\"#FF000000\">********* org Preview: ERROR GETTING TEXT FROM ICON SIZE ENTRY **********</span>");
        }
        if badsize_int == 0 {
                ...
                if treemodel1 == None {
                    ...
                    if let Some((modelt1, itert1)) = selectiont1.get_selected() {
                        ...
                        if errcd1 > 0 {
                            ...
                            let pixbufxc1 = Pixbuf::new_from_file(&fullnamec1).unwrap();
                            ...
                            let pixbuficonc1: Pixbuf = pixbufxc1.scale_simple(pixwidth1, pixheight1, gdk_pixbuf::InterpType::Bilinear).unwrap();
                            ocurrimage1_label.set_from_pixbuf(&pixbuficonc1);
                            if namep1 == " " {
                                oprevimage1_label.clear();
                            } else {
                                let fullnamep1 = format!("{}/{}", &str_cur_dir1, &namep1);
                                let pixbufxp1 = Pixbuf::new_from_file(&fullnamep1).unwrap();
                                ...
                                let pixbuficonp1: Pixbuf = pixbufxp1.scale_simple(pixwidth1, pixheight1, gdk_pixbuf::InterpType::Bilinear).unwrap();
                                oprevimage1_label.set_from_pixbuf(&pixbuficonp1);
                            }
                            if namea1 == " " {
                                oafterimage1_label.clear();
                            } else {
                                let fullnamea1 = format!("{}/{}", &str_cur_dir1, &namea1);
                                let pixbufxa1 = Pixbuf::new_from_file(&fullnamea1).unwrap();
                                ...
                                let pixbuficona1: Pixbuf = pixbufxa1.scale_simple(pixwidth1, pixheight1, gdk_pixbuf::InterpType::Bilinear).unwrap();
                                oafterimage1_label.set_from_pixbuf(&pixbuficona1);
                            }
                ...
                if treemodel2 == None {
                    ...
                    if let Some((modelt2, itert2)) = selectiont2.get_selected() {
                        ...
                        if errcd2 > 0 {
                            ...
                            let pixbufxc2 = Pixbuf::new_from_file(&fullnamec2).unwrap();
                            ...
                            let pixbuficonc2: Pixbuf = pixbufxc2.scale_simple(pixwidth2, pixheight2, gdk_pixbuf::InterpType::Bilinear).unwrap();
                            ocurrimage2_label.set_from_pixbuf(&pixbuficonc2);
                            if namep2 == " " {
                                oprevimage2_label.clear();
                            } else {
                                let fullnamep2 = format!("{}/{}", &str_cur_dir2, &namep2);
                                let pixbufxp2 = Pixbuf::new_from_file(&fullnamep2).unwrap();
                                ...
                                let pixbuficonp2: Pixbuf = pixbufxp2.scale_simple(pixwidth2, pixheight2, gdk_pixbuf::InterpType::Bilinear).unwrap();
                                oprevimage2_label.set_from_pixbuf(&pixbuficonp2);
                            }
                            if namea2 == " " {
                                oafterimage2_label.clear();
                            } else {
                                let fullnamea2 = format!("{}/{}", &str_cur_dir2, &namea2);
                                let pixbufxa2 = Pixbuf::new_from_file(&fullnamea2).unwrap();
                                ...
                                let pixbuficona2: Pixbuf = pixbufxa2.scale_simple(pixwidth2, pixheight2, gdk_pixbuf::InterpType::Bilinear).unwrap();
                                oafterimage2_label.set_from_pixbuf(&pixbuficona2);
                            }
...
// see if filesize exists and is between 4 and 16
        if bolok {
            if let Some(filesize_text) = ofilesize_entry.get_text() { 
                ...
                messageval_label.set_markup("<span color=\"#FF000000\">********* org Merge: ERROR GETTING TEXT FROM FILE LENGTH ENTRY **********</span>");
                bolok = false;
            }
        }
// validate date mod 1 & 2
        if bolok {
            if let Some(datemod1_text) = odatemod1_entry.get_text() { 
                let mut baddate1 = 0;
                ...
                messageval_label.set_markup("<span color=\"#FF000000\">********* org Merge: Date Mod 1 error in getting text **********</span>");
                bolok = false;
            }
        }
        if bolok {
            if let Some(datemod2_text) = odatemod2_entry.get_text() { 
                let mut baddate2 = 0;
                ...
                messageval_label.set_markup("<span color=\"#FF000000\">********* org Merge: Date Mod 2 error in getting text **********</span>");
                bolok = false;
            }
        }
        if bolok {
...
// check if both directories exist and they are not equal
        if bolok {
            if let Some(cur_dir1) = odirectory1_combobox.get_active_text() {
                str_cur_dir1 = cur_dir1;
                if let Some(cur_dir2) = odirectory2_combobox.get_active_text() {
                    str_cur_dir2 = cur_dir2;
                    if str_cur_dir1 == str_cur_dir2 {
...
// check if merge files and if so process
        if bolok {
            ...
            if view3model == None {
                ...
                while valid {
                      let dirval = view3modeluw.get_value(&validval,0);
            ...
            if treemodel1 == None {
                ...
                if let Some((modelt1, itert1)) = selectiont1.get_selected() {
                    ...
                    if !odir1chkbox_check.get_active() {
                        let takedate1 = modelt1.get_value(&itert1, 1);
                        let takedatestr1 = format!("{:?}", takedate1);
                        if takedatestr1.len() < 32 {
                            let imgdate1 = modelt1.get_value(&itert1, 2);
                            let imgdatestr1 = format!("{:?}", imgdate1);
                            if imgdatestr1.len() < 32 {
                                let moddate1 = modelt1.get_value(&itert1, 3);
                                let moddatestr1 = format!("{:?}", moddate1);
                                if moddatestr1.len() < 32 {
                ...
                if treemodel2 == None {
                    ...
                    if let Some((modelt2, itert2)) = selectiont2.get_selected() {
                        ...
                        if !odir2chkbox_check.get_active() {
                            let takedate2 = modelt2.get_value(&itert2, 1);
                            let takedatestr2 = format!("{:?}", takedate2);
                            if takedatestr2.len() < 32 {
                                let imgdate2 = modelt2.get_value(&itert2, 2);
                                let imgdatestr2 = format!("{:?}", imgdate2);
                                if imgdatestr2.len() < 32 {
                                    let moddate2 = modelt2.get_value(&itert2, 3);
                                    let moddatestr2 = format!("{:?}", moddate2);
                                    if moddatestr2.len() < 32 {
...
// validate date mod 1 & 2
        if bolok {
            if let Some(datemod1_text) = odatemod1_entry.get_text() { 
                ...
                messageval_label.set_markup("<span color=\"#FF000000\">********* org compare: Date Mod 1 error in getting text **********</span>");
                bolok = false;
            }
        }
        if bolok {
            if let Some(datemod2_text) = odatemod2_entry.get_text() { 
                ...
                messageval_label.set_markup("<span color=\"#FF000000\">********* org compare: Date Mod 2 error in getting text **********</span>");
                bolok = false;
            }
        }
        if bolok {
...
//----------------- Merge From directory button start -----------------------------------
    ...
    mdirectoryfrom_button.connect_clicked(clone!(window_weakmf, mdirectoryfrom_combobox_weakmf, mdirectoryto_combobox_weakmf, messageval_label_weakmf, miconfrom_view_weakmf, mimsize_entry_weakmf => move|_| {
        ...
        let mut badsize_int = 1;
        let mut icon_int1 = 0;
        if let Some(inputic_text) = mimsize_entry.get_text() { 
            let icon_int: i32 = inputic_text.parse().unwrap_or(-99);
            if icon_int > 0 {
                badsize_int = 0;
                icon_int1 = icon_int;
        ...
        if badsize_int == 0 {
            ...
            if result == RESPONSE_ACCEPT {
                if let Some(uri) = dialog.get_uri() {
                    let foldername = &uri[7..];
                    mdirectoryfrom_combobox.prepend_text(&foldername);
                    mdirectoryfrom_combobox.set_active(0);
                   ...
                   messageval_label.set_markup("<span color=\"#FF000000\">********* From Directory: ERROR GETTING URI **********</span>");
                }
            } else {
                messageval_label.set_markup("<span color=\"#FF000000\">********* From Directory: ERROR accept button not selected **********</span>");
            }
            dialog.destroy();
        }
    }));
    ...
    mdirectoryto_button.connect_clicked(clone!(window_weakmt, mdirectoryto_combobox_weakmt, messageval_label_weakmt, mtreeto_view_weakmt => move|_| {
        ...
        if result == RESPONSE_ACCEPT {
            if let Some(uri) = dialog.get_uri() {
                ...
                mdirectoryto_combobox.set_active(0);
                let current_dir = PathBuf::from(&foldername);
...
// check if from directory has images
        let iconmodel1 = miconfrom_view.get_model();
        if iconmodel1 == None {
...
// check if to directory has images
        if bolok {
            let treemodel1 = mtreeto_view.get_model();
            if treemodel1 == None {
...
// check if both directories exist and they are not equal
        if bolok {
            if let Some(cur_dir1) = mdirectoryfrom_combobox.get_active_text() {
                str_cur_dirfrom = cur_dir1;
                if let Some(cur_dir2) = mdirectoryto_combobox.get_active_text() {
                    str_cur_dirto = cur_dir2;
...
// date in name end
        ...
        if bolok {
            if datenumother > 999 {
                if mbeforebox_check.get_active() {
                    if datenumto < 2 {
                        messageval_label.set_markup("<span color=\"#FF000000\">********* Merge: selected file number is less than 2, first file and before is checked **********</span>");
                        bolok = false;
                    } else {
                ...
                if dateto == dateother {
                    if mbeforebox_check.get_active() {
                        if (datenumto - datenumother) < 2 {
                            messageval_label.set_markup("<span color=\"#FF000000\">********* Merge: before checked & selected file number & previous file number less than 2 apart **********</span>");
                            bolok = false;
                        } else {
                        ...
                        if (datenumother - datenumto) < 2 {
                            messageval_label.set_markup("<span color=\"#FF000000\">********* Merge: selected file number & next file number less than 2 apart **********</span>");
                            bolok = false;
                        } else {
                    ...
                    if !bolusenum {
                        ...
                        if durationn.num_seconds() < 2 {
                            if mbeforebox_check.get_active() {
                                if datenumother > 998 {
                                    messageval_label.set_markup("<span color=\"#FF000000\">********* Merge: before checked & selected file number & previous file number too close **********</span>");
                                    bolok = false;
                                } else {
                                    dateto = dateother;
...
//----------------- Merge preview button start -----------------------------------
    ...
    mpreview_button.connect_clicked(clone!(mdirectoryto_combobox_weakmp, mtreeto_view_weakmp, mprevimageto_label_weakmp, mcurrimageto_label_weakmp, mafterimageto_label_weakmp, messageval_label_weakmp, mimsize_entry_weakmp  => move|_| {
        ...
        if let Some(inputic_text) = mimsize_entry.get_text() { 
            ...
            messageval_label.set_markup("<span color=\"#FF000000\">********* Preview: ERROR GETTING TEXT FROM ICON SIZE ENTRY **********</span>");
        }
        if badsize_int == 0 {
            if (icon_int1 < 50) | (icon_int1 > 255) {
                ...
                if treemodel1 == None {
                    ...
                    if let Some((modelt, itert)) = selectiont.get_selected() {
                        ...
                        if errcd > 0 {
                            ...
                            let pixbufxc = Pixbuf::new_from_file(&fullnamec).unwrap();
                            ...
                            let pixbuficonc: Pixbuf = pixbufxc.scale_simple(pixwidth, pixheight, gdk_pixbuf::InterpType::Bilinear).unwrap();
                            mcurrimageto_label.set_from_pixbuf(&pixbuficonc);
                            if namep == " " {
                                mprevimageto_label.clear();
                            } else {
                                let fullnamep = format!("{}/{}", &str_cur_dir2, &namep);
                                let pixbufxp = Pixbuf::new_from_file(&fullnamep).unwrap();
                                ...
                                let pixbuficonp: Pixbuf = pixbufxp.scale_simple(pixwidth, pixheight, gdk_pixbuf::InterpType::Bilinear).unwrap();
                                mprevimageto_label.set_from_pixbuf(&pixbuficonp);
                            }
                            if namea == " " {
                                mafterimageto_label.clear();
                            } else {
                                let fullnamea = format!("{}/{}", &str_cur_dir2, &namea);
                                let pixbufxa = Pixbuf::new_from_file(&fullnamea).unwrap();
                                ...
                                let pixbuficona: Pixbuf = pixbufxa.scale_simple(pixwidth, pixheight, gdk_pixbuf::InterpType::Bilinear).unwrap();
                                mafterimageto_label.set_from_pixbuf(&pixbuficona);
                            }
    ...
    cdirectory1_button.connect_clicked(clone!(window_weakcd, cdirectory1_combobox_weakcd, messageval_label_weakcd, ctree_view1_weakcd => move|_| {
        ...
        if result == RESPONSE_ACCEPT {
            if let Some(uri) = dialog.get_uri() {
                ...
                cdirectory1_combobox.set_active(0);
                let current_dir = PathBuf::from(&foldername);
                let (errcd, errstr, newmodel) = get_dirmodel(current_dir);
                if errcd == 0 {
    ...
    cdirectory_o_button.connect_clicked(clone!(window_weakcd, cdirectory_o_combobox_weakcd, messageval_label_weakcd => move|_| {
        ...
        if result == RESPONSE_ACCEPT {
            if let Some(uri) = dialog.get_uri() {
                ...
                cdirectory_o_combobox.set_active(0);
...
// see if filesize exists and is between 4 and 16
        if bolok {
            if let Some(filesize_text) = cfilesize_entry.get_text() { 
                filesize_int = filesize_text.parse().unwrap_or(-99);
                if filesize_int > 0 {
                    if (filesize_int < 4) | (filesize_int > 16) {
                ...
                messageval_label.set_markup("<span color=\"#FF000000\">********* convert Merge: ERROR GETTING TEXT FROM FILE LENGTH ENTRY **********</span>");
                bolok = false;
            }
        }
// validate date mod 
        if bolok {
            if let Some(datemod1_text) = cdatemod1_entry.get_text() { 
                let datemod1_textx: &String = &format!("{}", datemod1_text);
                let date1ar1: Vec<&str> = datemod1_textx[0..].split(":").collect();
                let lendat1 = date1ar1.len();
                if (lendat1 > 6) | (lendat1 < 6) {
                ...
                messageval_label.set_markup("<span color=\"#FF000000\">********* convert Merge: Date Mod error in getting text **********</span>");
                bolok = false;
            }
        }
        if bolok {
...
//----------------- convert copy button start -----------------------------------
    ...
    ccopy_button.connect_clicked(clone!(cdirectory1_combobox_weakcc, ctree_view3_weakcc, progress_progressbar_weakcc, messageval_label_weakcc  => move|_| {
...
// check if merge files and if so process
        if bolok {
            let view3model = ctree_view3.get_model();
            if view3model == None {
